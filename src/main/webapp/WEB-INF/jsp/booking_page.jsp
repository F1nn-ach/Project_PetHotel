...
<link rel="stylesheet" type="text/css"
	href="assets/css/style_booking.css">
</head>
<body>
	...
	<div class="container">
		...
		<form action="booking" method="post" id="bookingForm">
			...
			<div class="form-group">
				...
				<c:forEach items="${roomTypes}" var="roomType">
					<div class="room-type-card"
						onclick="selectRoomType(${roomType.roomTypeId})">
						<input type="radio" name="roomTypeId"
							value="${roomType.roomTypeId}" required>
						<h3>${roomType.roomTypeName}</h3>
						<p>${roomType.description}</p>
						<p class="price">Price: ฿${roomType.roomTypePrice} per night</p>
						<p>Available Rooms:
							${vacantRoomCounts[roomType.roomTypeId]}</p>
					</div>
				</c:forEach>
			</div>

			<div class="form-group">
				<label for="startDate">Check-in Date</label> <input type="date"
					id="startDate" name="startDate" required min="${today}">
			</div>

			<div class="form-group">
				<label for="startTime">Check-in Time</label> <select id="startTime"
			...
			<div class="form-group">
				<label for="endDate">Check-out Date</label> <input type="date"
					id="endDate" name="endDate" required min="${today}">
			</div>

			<div class="form-group">
				<label for="endTime">Check-out Time</label> <select id="endTime"
