...
public class HotelManager {
	...
	public Pet getPetByBookingId(String bookingId) {
		Pet pet = null;
		SessionFactory sessionFactory = HibernateConnection.doHibernateConnection();
		Session session = sessionFactory.openSession();
		Transaction tx = null;
		...
		try {
			tx = session.beginTransaction();
			// เปลี่ยนการค้นหาจาก pet_id เป็นการดึง Pet object โดยตรง
			String hql = "SELECT p FROM Booking b JOIN b.pet p WHERE b.id = :bookingId";
			pet = session.createQuery(hql, Pet.class).setParameter("bookingId", bookingId).uniqueResult();
			tx.commit();
		} catch (Exception ex) {
			if (tx != null) {
				tx.rollback();
